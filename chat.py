...
def main():
    ...
    while number != '6':
        if number == '1':
            ...
            list_all_user(username, user_role2)
            print_menu()
            number = input()
        ...
        if role_picked not in roles_available:
            ...
            if role_picked == '3':
                ...
                if "moderator" in user_role2:
                    print("You are already registered as a moderator you cannot lower your role.")
    else:
        print("Please enter a username which exists already or the username you used when you registered.")
    return(roles(username, user_role2))


def list_all_user(username, user_role2):
    print("You have selected 'list all users'.")
    for user in username and user_role2:
        print(username and user_role2)
    return(list_all_user(username, user_role2))
